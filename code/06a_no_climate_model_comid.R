...
load(file = "ignore/04_all_env_bio_data_NHD_COMID.RData")
...
all_data_obs <- NewDataObsSub %>%
  ...
  mutate(value = ifelse(value < 0, 0, value)) %>% ## check this and change of needed
  pivot_wider(names_from = "ffm", values_from = "value") %>%
  select(COMID, PresAbs, TC_042014_RB9.1_Med:TC_042014_RB9.3_Var,AvgClay:VRM9.Mn,Catchment.A, MRVBF.Mx, DS_Mag_50:Wet_BFL_Mag_50 ) %>%
  drop_na()

head(all_data_obs)
names(all_data_obs)

cl <- MultiColinear(all_data_obs[,c(3:36)], p=0.05)
xdata <- all_data_obs[,c(3:36)]
xdata
...
xdata <- all_data_obs[,c(3:35)] 

class(ydata)
length(ydata)

# PERCENT OF PRESENCE OBSERVATIONS
( dim(all_data_obs[all_data_obs$PresAbs == 1, ])[1] / dim(all_data_obs)[1] ) * 100 ## 74%
...
(rf.final <- randomForest(y=rf.data[,1], x=rf.data[,2:ncol(rf.data)], ntree=b, mtry = 5, nodesize=5,
...
file.name1 <- "/Users/katieirving/OneDrive - SCCWRP/Documents - Katie’s MacBook Pro/git/Arroyo_toad_RB9/Figures/01_no_climate_model_mds_scale_comid.jpg"
...
library(rpart)
getwd()
pdf("/Users/katieirving/OneDrive - SCCWRP/Documents - Katie’s MacBook Pro/git/Arroyo_toad_RB9_V2/Figures/01_Trees_comid_no_climate_model.pdf", width=25, height=15)
...
sp=0.6
pdf("/Users/katieirving/OneDrive - SCCWRP/Documents - Katie’s MacBook Pro/git/Arroyo_toad_RB9_V2/Figures/PartialPlots_6_no_climate_model_comid.pdf", width=8, height=8)
p <- as.matrix(rf.final$importance)    
ord <- rev(order(p[,1], decreasing=TRUE)[1:dim(p)[1]])  
dotchart(p[ord,1], main="Scaled Variable Importance", pch=19)
plot(rf.final, main="Bootstrap Error Convergence")
...
st_write(nhd_lines_probx, "output_data/full_model/01_no_climate_model_prob_occurrence_comid.shp", layer_options = "OVERWRITE=true")
...
file.name1 <- "/Users/katieirving/Documents/Documents - Katie’s MacBook Pro/git/Arroyo_toad_RB9/Figures/01_no_climate_model_comid_prob_occs_map.jpg"
...
mapshot(m1, url = paste0(getwd(), "/ignore/01_no_climate_model_comid_prob_occs_mapview.html"),
        file = paste0(getwd(), "/ignore/01_no_climate_model_comid_prob_occs_mapview.png"))
getwd()
